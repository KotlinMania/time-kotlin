...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`time`](https://crates.io/crates/time).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `time` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Date and time crate (alternative to chrono)
...
Original work copyrighted by the upstream `time` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
