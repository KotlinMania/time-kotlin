# Claude Code Project Instructions — time-kotlin

## Project Overview

This is **time-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`time`](https://crates.io/crates/time).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/time/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/time>
